...
class TestLLMTTE:
    """Test TTE-aware LLM fallback behavior."""

    def test_fallback_brief_includes_primary_tte_and_urgency(self):
        from llm.explainer import LLMExplainer

        explainer = LLMExplainer()
        text = explainer._fallback_brief(
            node_id="N1",
            severity_band="critical",
            primary_metric="throughput_risk",
            primary_probability=0.92,
            capacity_eta_min=float("inf"),
            primary_metric_eta_min=4.0,
            confidence_score=0.88,
            domain_hints=[{"domain": "transport", "score": 0.9}],
            temporal_signals={"risk_velocity": 0.2},
            prepared_incidents=[{"incident_type": "low_throughput", "severity": "critical", "similarity_pct": 90.0, "summary": "prior"}],
        )

        assert "4.0 min" in text
        assert "IMMEDIATE" in text
        assert "throughput_risk" in text
