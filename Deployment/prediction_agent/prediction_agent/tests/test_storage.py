...
import storage.results_store as results_store_module
...
class TestResultsStore:
    ...
    def test_feedback_roundtrip(self, store, sample_prediction):
        """Test storing and retrieving operator feedback."""
        pred_id = store.store_prediction(sample_prediction)
        feedback_id = store.add_feedback(
            pred_id,
            feedback_type="confirmed",
            outcome_status="true_positive",
            notes="Matched live NOC observation",
        )
        assert feedback_id > 0
        feedback = store.get_feedback_for_prediction(pred_id)
        assert len(feedback) == 1
        assert feedback[0]["feedback_type"] == "confirmed"

    def test_runtime_db_marker_is_persisted_and_reused(self, monkeypatch, tmp_path: Path):
        primary = tmp_path / "primary.db"
        marker = tmp_path / "predictions_runtime_path.txt"

        monkeypatch.setattr(results_store_module, "_get_runtime_db_marker_path", lambda: marker)

        first = ResultsStore(primary)
        assert marker.exists()
        assert marker.read_text(encoding="utf-8").strip() == str(primary)

        alt = tmp_path / "alternate.db"
        marker.write_text(str(alt), encoding="utf-8")
        second = ResultsStore()
        assert second.db_path == alt
